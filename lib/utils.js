...
/*0910
    if (!dataItem.enriched_text ||
      ...
      console.log('     dataItem.title: ' + dataItem.不具合装置);
...
*/
    ...
    var initialTextLen = (dataItem.申告内容.length > 100)? 100 : Math.ceil(dataItem.申告内容.length / 2);
    // make sure we end between words
    initialTextLen = dataItem.申告内容.lastIndexOf(' ', initialTextLen);
...
//for debug
    ...
    if (!dataItem.result_metadata.confidence){
      console.log('Add confidence to 1.0');
      dataItem.result_metadata.confidence = '1.0';
    }
    ...
    var newResult = {
      ...
      title: dataItem.不具合装置,
//0910      title: dataItem.title,
      // text: dataItem.text,
      textBlurb: dataItem.申告内容.substr(0, initialTextLen),
      textFull: dataItem.申告内容,
//0910      textBlurb: dataItem.text.substr(0, initialTextLen),
//0910      textFull: dataItem.text,
      date: dataItem.日付,
//0910      date: dataItem.date,
      score: dataItem.result_metadata.confidence,
    ...
    if (passages.results) {
      ...
      for (var i=0; i<passages.results.length; i++) {
        ...
        if (res.document_id === dataItem.番号) {
      ...
      if (dataItem.highlight) {
        if (dataItem.highlight.申告内容) {
          // text highlights
          processHighLights(dataItem.highlight.申告内容, dataItem.申告内容, newResult, 'text');
        }
        if (dataItem.highlight.不具合装置) {
          // title highlights
          processHighLights(dataItem.highlight.不具合装置, dataItem.不具合装置, newResult, 'title');
        }
      }
          ...
          if (typeof entity.mentions !== 'undefined' && entity.mentions.length > 0) {
            entity.mentions.forEach(function(mention) {
              ...
              var highlightText = 'enriched_text.entities.text::"' +
                dataItem.申告内容.substr(mention.location[0], mention.location[1] - mention.location[0]) + '"';
...
*/
      ...
      if (typeof filterString !== 'undefined' && filterString.length > 1) {
        // we only care if we have a match on our filter values
        dataItem.車名.forEach(function(entity) {
          if (typeof 車名 !== 'undefined' && 車名.length > 0) {
            車名.forEach(function(mention) {
              // get the text that will be highlighted and put it in the
              // same format as the filters passed in the query
              // string sent to discovery
              var highlightText = '車名::"' +
                dataItem.申告内容.substr(mention.location[0], mention.location[1] - mention.location[0]) + '"';
